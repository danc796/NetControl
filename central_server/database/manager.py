...
class DatabaseManager:
    ...
    def register_server(self, ip_address, port, client_ip):
        """Register a server with updated sharing_with in servers table"""
        # Method signature remains compatible with existing calls
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            # Since we're in DatabaseManager class, we don't have direct access to self.clients
            # The username needs to be passed from ConnectionManager
            # For now, we'll use a default empty string, and the caller should update it if needed
            username = ""

            # Update the server record with last_seen and sharing_with
            cursor.execute(
                """
                UPDATE servers 
                SET last_seen = ?, sharing_with = ? 
                WHERE server_id = ?
                """,
                (datetime.now(), username, server_id)
            )
