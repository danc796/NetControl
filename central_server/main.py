...
def main():
    ...
    # Determine server host, port, and SSL setting
    host = args.address or config.get_server_host()
    port = args.port or config.get_server_port()
    use_ssl = not args.no_ssl and config.get_use_ssl()
    ...
    try:
        logging.info(f"Starting NetControl Central Server on {host}:{port} (SSL: {use_ssl})")
        server.start()
