...
class ConnectionManager:
    ...
    def handle_client(self, client_socket, address):
        ...
        try:
            ...
            if client_info and client_info.get('username'):
                ...
                client_ip = address[0]
                # Modified this section to not rely on active_connections table

                try:

                    # Find all servers where this user is the most recent connection
                    conn = self.database.get_connection()
                    cursor = conn.cursor()

                    # Update the sharing_with field for any server this user was connected to
                    cursor.execute(
                        "UPDATE servers SET sharing_with = '' WHERE sharing_with = ?",
                        (username,)
                    )
                    ...
                    conn.commit()
                    conn.close()
                    log_connection(address, username, f"disconnected and cleared sharing for {count} servers")
    ...
    def handle_register_server(self, data, address):
        ...
        # Updated to pass only 3 arguments instead of 4:
        success, message = self.database.register_server(ip_address, port, client_ip)

        if success:
            # Now update the sharing_with field with the username
            try:
                conn = self.database.get_connection()
                cursor = conn.cursor()
                ...
                if server_record:
                    ...
                    # Update the sharing_with field
                    cursor.execute(
                        "UPDATE servers SET sharing_with = ? WHERE server_id = ?",
                        (username, server_id)
                    )
                    conn.commit()
                ...
                logging.error(f"Error updating sharing_with for server: {e}")
    ...
    def handle_get_connection_history(self, data, address):
        """Handle request for connection history for a specific server without using active_connections"""
        user_id = self.clients[address]['user_id']
        username = self.clients[address]['username']
        ip_address = data.get('ip_address')
        port = data.get('port')
        ...
        try:
            ...
            cursor.execute(
                "SELECT server_id, last_seen FROM servers WHERE ip_address = ? AND port = ?",
                (ip_address, port)
            )
            server_record = cursor.fetchone()
            ...
            # Since we're not using active_connections, we'll create a single record
            # with the server's last connection information
            connection_history = [{
                'server_id': server_id,
                'connection_time': last_seen,
                'connect_by': 'Unknown',  # We don't track this without active_connections
                'recent_connection': username  # Use current user as the most recent connection
            }]
    ...
    def handle_get_server_connection_info(self, data, address):
        """Handle request for server connection info using servers table with sharing_with column"""
        user_id = self.clients[address]['user_id']
        username = self.clients[address]['username']
        ip_address = data.get('ip_address')
        port = data.get('port')
        client_username = data.get('username')  # Current user's username
        ...
        try:
            ...
            # Find the server
            cursor.execute(
                "SELECT server_id, last_seen, sharing_with FROM servers WHERE ip_address = ? AND port = ?",
                (ip_address, port)
            )
            server_record = cursor.fetchone()
            ...
            server_id = server_record['server_id']
            last_seen = server_record['last_seen']
            sharing_with = server_record['sharing_with'] if 'sharing_with' in server_record else None

            # Update the server's last_seen time and sharing_with field
            # The sharing_with field will store the username of the most recent connection
            current_time = datetime.now()
            cursor.execute(
                "UPDATE servers SET last_seen = ?, sharing_with = ? WHERE server_id = ?",
                (current_time, client_username, server_id)
            )

            conn.commit()

            # Create the connection info record
            connection_info = [{
                'server_ip': ip_address,
                'recent_connection': client_username,  # Current user becomes the most recent connection
                'connected_at': current_time.strftime("%Y-%m-%d %H:%M:%S")
            }]

            # If there was a previous sharing_with value, add it as a historical record
            if sharing_with and sharing_with != client_username:
                # Add historical record with the previous connection info
                connection_info.append({
                    'server_ip': ip_address,
                    'recent_connection': sharing_with,  # Previous user
