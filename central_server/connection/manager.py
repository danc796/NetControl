...
class ConnectionManager:
    def __init__(self, host='0.0.0.0', port=5001):
        ...
        self.command_handlers = {
            ...
            'get_connection_history': self.handle_get_connection_history,
            # User Management
    ...
    def handle_get_server_connection_info(self, data, address):
        """Handle request for server connection info - READ-ONLY operation"""
        user_id = self.clients[address]['user_id']
        username = self.clients[address]['username']
        ip_address = data.get('ip_address')
        port = data.get('port')
        ...
        # Get connection info from the servers table - READ-ONLY operation
        try:
            conn = self.database.get_connection()
            cursor = conn.cursor()

            # Find the server - just SELECT, no UPDATE
            cursor.execute(
            ...
            # Create connection info response - NO DATABASE MODIFICATIONS
            connection_info = []

            # Always show the current recent_connection as-is
    ...
    def handle_get_connection_details(self, data, address):
        """Handle request for detailed connection information (admin only)"""
        user_id = self.clients[address]['user_id']
        username = self.clients[address]['username']
        ...
        # Get connection details with the actual username
    ...
    def handle_get_connection_history(self, data, address):
        ...
        try:
            ...
            # Find the server_id and connection info
            cursor.execute(
                "SELECT server_id, last_seen, recent_connection FROM servers WHERE ip_address = ? AND port = ?",
                (ip_address, port)
            )
            ...
            # Create connection history record with actual username
            connection_history = [{
                'server_id': server_id,
