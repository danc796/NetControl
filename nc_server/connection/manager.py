...
class ConnectionManager:
    def __init__(self, host='0.0.0.0', port=5000, use_ssl=False):
        """Initialize the connection manager (encryption disabled)"""
        logging.info("Initializing nc Server Connection Manager (encryption disabled)")
        ...
        # Create a properly formatted Fernet key (for compatibility)
        # This is a valid Fernet key format but we won't use it for actual encryption
        self.encryption_key = b'dummyKeyThatIsExactly32BytesLongs='

        # Use the simplified encryption manager (no actual encryption)
        self.encryption_manager = EncryptionManager(use_ssl=False)
        self.encryption_manager.encryption_key = self.encryption_key
    ...
    def start(self):
        ...
        try:
            ...
            logging.info(f"Server started on {self.host}:{self.port} (encryption disabled)")
    ...
    def handle_client(self, client_socket, address):
        """Handle client connection without SSL/encryption"""
        try:
            # No SSL wrapping needed, use socket directly
            wrapped_socket = client_socket
            logging.info(f"Connection established with {address} (encryption disabled)")

            # Set a timeout for operations
            wrapped_socket.settimeout(5.0)

            # Send dummy certificate size (0 bytes)
            wrapped_socket.send((0).to_bytes(4, byteorder='big'))

            # Send properly formatted dummy Fernet key
            ...
            while self.running:
                try:
                    # Receive data directly (no encryption)
                    ...
                    try:
                        ...
                        # Send response directly (no encryption)
                        response_json = json.dumps(response)
                        wrapped_socket.send(response_json.encode())
