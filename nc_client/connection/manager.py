...
class ConnectionManager:
    ...
    def connect_to_server(self, connection_id):
        """Connect to server with RSA + AES hybrid encryption and auto-register with central server"""
        ...
        try:
            ...
            logging.info(f"Connection established with {host}:{port} using RSA + AES hybrid encryption")

            # Create a new encryption manager for each connection
            encryption_manager = EncryptionManager()

            # Perform RSA + AES key exchange
            if not encryption_manager.perform_key_exchange(wrapped_socket):
                logging.warning(f"Key exchange failed for {host}:{port}, continuing with fallback encryption")
    ...
    def auto_register_with_central_server(self, connection_id):
        ...
        try:
            ...
            # Register the server (this will automatically handle sharing via the register_server method)
            success, message = self.parent_app.central_client.register_server(host, port)

            if success:
                logging.info(f"Successfully registered and shared server {host}:{port} with central server")
    ...
    def attempt_reconnection(self, connection_id):
        """Attempt to reconnect to a failed connection with RSA + AES hybrid encryption"""
        connection = self.connections.get(connection_id)
        if not connection:
            return False
        ...
        try:
            ...
            if not encryption_manager.perform_key_exchange(wrapped_socket):
                logging.warning(
                    f"Key exchange failed on reconnect for {host}:{port}, continuing with fallback encryption")
