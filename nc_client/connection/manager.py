...
class ConnectionManager:
    def __init__(self, parent_app, use_ssl=False):
        self.parent_app = parent_app
        self.connections = {}
        self.use_ssl = False  # Force SSL off

        # Initialize encryption manager (simplified - no actual encryption)
        self.encryption_manager = EncryptionManager(use_ssl=False)
    ...
    def connect_to_server(self, connection_id):
        """Connect to server without SSL/encryption"""
        ...
        try:
            ...
            # No SSL wrapping needed
            wrapped_socket = client_socket
            logging.info(f"Connection established with {host}:{port} (encryption disabled)")

            # Receive dummy certificate size for protocol compatibility
            ...
            # Get encryption key - just store it, don't create a real Fernet cipher
            key = wrapped_socket.recv(44)
            self.encryption_manager.set_encryption_key(key)

            # Store the encryption manager as the dummy cipher suite
            cipher_suite = self.encryption_manager
    ...
    def send_command(self, connection_id, command_type, data):
        ...
        try:
            ...
            # Send data directly without encryption
            connection['socket'].send(json_data.encode())
            print("Command sent successfully")

            # Receive response with timeout handling
            try:
                print("Waiting for response...")
                response_data = connection['socket'].recv(16384)
                ...
                print(f"Received response of length: {len(response_data)}")

                # Parse the response directly (no decryption)
                try:
    ...
    def attempt_reconnection(self, connection_id):
        """Attempt to reconnect to a failed connection without SSL"""
        connection = self.connections.get(connection_id)
        if not connection:
            return False
        ...
        try:
            ...
            # Get encryption key for protocol compatibility - don't create a real Fernet cipher
            key = wrapped_socket.recv(44)
            self.encryption_manager.set_encryption_key(key)

            # Update connection info
            self.connections[connection_id]['socket'] = wrapped_socket
            self.connections[connection_id]['cipher_suite'] = self.encryption_manager
            self.connections[connection_id]['connection_active'] = True
