...
class SystemMonitor:
    ...
    def update_hardware_info(self, data):
        ...
        try:
            # Update CPU usage
            self._update_cpu_usage(data)

            # Update memory usage
            self._update_memory_usage(data)

            # Update disk usage
            self._update_disk_usage(data)
    ...
    def _update_cpu_usage(self, data):
    ...
    def _update_memory_usage(self, data):
    ...
    def _update_disk_usage(self, data):
    ...
    def update_disk_info(self, disk_usage):
        ...
        try:
            ...
            # Clear existing disk information and create new display
            self._create_disk_display(disk_frame, disk_usage)

        except Exception as e:
            print(f"Error updating disk info: {str(e)}")

    def _create_disk_display(self, disk_frame, disk_usage):
        """Create new disk display"""
        ...
        for mount, usage in disk_usage.items():
            ...
            try:
                self._create_disk_entry(disk_frame, mount, usage)
            except Exception as disk_error:
                print(f"Error displaying disk {mount}: {str(disk_error)}")
                continue

    def _create_disk_entry(self, disk_frame, mount, usage):
        """Create a single disk entry"""
        ...
        try:
            ...
            for i, (mount, usage) in enumerate(disk_usage.items()):
                ...
                try:
                    ...
                    # Update values only (drive name should stay the same)
                    # [0] = Drive name label
                    # [1] = Total capacity
                    total_gb = total / (1024 ** 3)
                    if len(labels) > 1:
