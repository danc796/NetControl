...
class CentralServerTab:
    ...
    def show_connections_dialog(self, ip_address, port):
        """Show a dialog with connection info for a server - FIXED: Shows First Discovered, Last Seen, Recent Connection"""
        ...
        # Create Treeview for connection info - FIXED: No connection_time, added first_discovered and last_seen
        columns = ("first_discovered", "last_seen", "recent_connection")
        connections_tree = ttk.Treeview(main_frame, columns=columns, show="headings", height=10)

        # Define column headings
        connections_tree.heading("first_discovered", text="First Discovered")
        connections_tree.heading("last_seen", text="Last Seen")
        connections_tree.heading("recent_connection", text="Recent Connection")

        # Configure column widths
        connections_tree.column("first_discovered", width=200)
        connections_tree.column("last_seen", width=200)
        connections_tree.column("recent_connection", width=200)
        ...
        refresh_button = ctk.CTkButton(
            button_frame,
            text="Refresh",
            command=lambda: self.load_connection_info_fixed(connections_tree, status_label, ip_address, port)
        )
        ...
        self.load_connection_info_fixed(connections_tree, status_label, ip_address, port)

    def load_connection_info_fixed(self, tree, status_label, ip_address, port):
        """Load connection info with server details - FIXED to get server data properly"""
        # Clear existing items
        ...
        # Get server details from the server list (which has all the data we need)
        success, message, servers = self.app.central_client.get_all_servers_from_db()
        ...
        if success and servers:
            ...
            for server in servers:
                if (str(server.get('ip_address')) == str(ip_address) and
                        str(server.get('port')) == str(port)):
                    target_server = server
                    break

            if target_server:
                # Get recent connection info
                recent_connection = target_server.get('recent_connection', 'No recent connection')
                if not recent_connection or recent_connection.strip() == '':
                    recent_connection = 'No recent connection'

                # Add the server info to the tree
                tree.insert(
                    "",
                    tk.END,
                    iid="server_info",
                    values=(
                        target_server.get('first_discovered', 'Unknown'),
                        target_server.get('last_seen', 'Unknown'),
                        recent_connection
                    )
                )
                ...
                status_label.configure(text="Server not found in database")
                tree.insert(
                    "",
                    tk.END,
                    iid="error",
            ...
            error_message = message if not success else "No server data found"
            status_label.configure(text=f"Error: {error_message}")

            # If there's an error, show a placeholder entry
            tree.insert(
                "",
                tk.END,
                iid="error",
    ...
    def load_connection_info(self, tree, status_label, ip_address, port):
        """Load connection info for a server from the servers table with recent_connection"""
        # Clear existing items
        for item in tree.get_children():
            tree.delete(item)
        ...
        # Get connection info from central server (read-only operation)
        ...
        if success and info:
            ...
            for i, entry in enumerate(info):
                # Handle the recent_connection field properly
                recent_connection = entry.get('recent_connection', 'No recent connection')
                connected_at = entry.get('connected_at', 'Unknown')
                server_ip = entry.get('server_ip', ip_address)
            ...
            status_label.configure(text=f"Connection information loaded successfully ({len(info)} records)")
        else:
            error_message = message if not success else "No connection information found"
            status_label.configure(text=f"Error: {error_message}")
