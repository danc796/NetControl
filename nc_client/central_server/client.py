...
class CentralServerClient:
    def __init__(self, parent_app, host=None, port=5001, use_ssl=True):
        """Initialize the central server client"""
        ...
        while retry_count < self.max_retries:
            try:
                ...
                if self.use_ssl:
                    ...
                    wrapped_socket = context.wrap_socket(client_socket, server_hostname=self.host)

                    # Receive certificate (size + data)
                    ...
                    if cert_size > 0:
                        ...
                        logging.warning("Central server did not provide a certificate")

                    # Get encryption key
                    self.encryption_key = wrapped_socket.recv(44)
                    self.cipher_suite = Fernet(self.encryption_key)

                    # Use the wrapped socket
                    self.socket = wrapped_socket
                else:
                    # Use the plain socket
                    self.socket = client_socket

                self.connected = True
                logging.info(f"Connected to central server at {self.host}:{self.port}")
    ...
    def register_server(self, ip_address, port):
        """Register a server connection with the central server"""
        if not self.authenticated:
    ...
    def send_command(self, command_type, data):
        """Send a command to the central server"""
        ...
        try:
            ...
            if self.cipher_suite:
                ...
                # Receive response
                encrypted_response = self.socket.recv(16384)
                ...
                # This should only happen if SSL is disabled
                # In a production environment, you would want to implement
                # non-SSL communication here
                logging.error("No cipher suite available")
                self.connected = False
                return None
        except Exception as e:
