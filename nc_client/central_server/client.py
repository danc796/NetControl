...
class CentralServerClient:
    ...
    def connect(self):
        """Connect to the central server with RSA + AES hybrid encryption"""
        if not self.host:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                if not self.encryption_manager.perform_key_exchange(wrapped_socket):
                    logging.warning("Key exchange failed, continuing with fallback encryption")
                ...
                self.connected = True
                logging.info(f"Connected to central server at {self.host}:{self.port} with RSA + AES hybrid encryption")
    ...
    def auto_register_existing_connections(self):
        ...
        for conn_id, connection in connections.items():
            if connection.get('connection_active', False):
                ...
                # Don't pass client_ip - the server will determine this from the connection
    ...
    def register_server(self, ip_address, port):
        ...
        # Don't include client_ip - the server will determine this from the connection
    ...
    def unregister_server(self, ip_address, port):
        ...
        try:
            response = self.send_command("some_command", {
                "parameter": "value"
            })
            ...
            # We don't delete it, just clear the sharing_with and client_ip fields
            cursor.execute(
                """
                UPDATE servers 
                SET sharing_with = '', client_ip = '' 
                WHERE server_id = ?
                """,
                (server_id,)
            )
    ...
    def set_connection_sharing(self, ip_address, port, is_shared, username):
        """Set whether a connection is shared by updating the sharing_with field"""
        try:
            response = self.send_command("some_command", {
                "parameter": "value"
            })
            ...
            # Get the server_id
            cursor.execute(
                "SELECT server_id, sharing_with FROM servers WHERE ip_address = ? AND port = ?",
                (ip_address, port)
            )
            server_record = cursor.fetchone()
    ...
    def get_all_servers_from_db(self):
        """Get a list of all servers from the 'servers' table in the database"""
        if not self.authenticated:
            return False, "Not authenticated", []

        # Simply send command to server - don't try to access database directly
        response = self.send_command("get_all_servers_from_db", {})

        if response and response.get("status") == "success":
            servers = response.get("data", [])
            return True, f"Found {len(servers)} servers", servers
        else:
            error_msg = response.get("message", "Failed to get servers") if response else "No response from server"
            logging.error(f"Getting servers failed: {error_msg}")
            return False, error_msg, []

    def get_connections_for_server(self, ip_address, port):
        """Get connections for a specific server"""
        if not self.authenticated:
            return False, "Not authenticated", []

        response = self.send_command("get_connections_for_server", {
            "ip_address": ip_address,
            "port": port
        })
    ...
    def delete_connection(self, server_id):
        """Delete a connection by server_id"""
        if not self.authenticated or not self.is_admin:
            return False, "Admin privileges required"

        response = self.send_command("delete_connection", {
            "server_id": server_id
        })

        if response and response.get("status") == "success":
            logging.info(f"Deleted connection for server_id: {server_id}")
            return True, "Connection deleted successfully"
        else:
            error_msg = response.get("message",
                                     "Failed to delete connection") if response else "No response from server"
            logging.error(f"Connection deletion failed: {error_msg}")
            return False, error_msg

    def get_connection_history(self, ip_address, port):
        """Get connection history for a specific server without using active_connections table"""
        if not self.authenticated:
            return False, "Not authenticated", []

        response = self.send_command("get_connection_history", {
            "ip_address": ip_address,
            "port": port
        })
        ...
        if response and response.get("status") == "success":
            ...
            return True, f"Found {len(history)} connection records", history
        else:
            error_msg = response.get("message",
                                     "Failed to get connection history") if response else "No response from server"
            logging.error(f"Getting connection history failed: {error_msg}")
            return False, error_msg, []

    def get_server_connection_info(self, ip_address, port):
        """Get connection info for a specific server from the servers table with sharing_with column"""
        if not self.authenticated:
            return False, "Not authenticated", []

        # Send a command to get the server connection info and update the recent connection
        # Remove client_ip reference that was causing yellow lines
        response = self.send_command("get_server_connection_info", {
            "ip_address": ip_address,
            "port": port,
            "username": self.username  # Send current username to update recent connection
        })

        if response and response.get("status") == "success":
            info = response.get("data", [])
            return True, "Connection info retrieved successfully", info
        else:
            error_msg = response.get("message",
                                     "Failed to get connection info") if response else "No response from server"
            logging.error(f"Getting server connection info failed: {error_msg}")
            return False, error_msg, []