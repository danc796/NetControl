...
class CentralServerClient:
    ...
    def register_server(self, ip_address, port):
        ...
        if response and response.get("status") == "success":
            ...
            # Always share the server (the server will automatically set recent_connection)
    ...
    def unregister_server(self, ip_address, port):
        ...
        if response and response.get("status") == "success":
            ...
            error_msg = response.get("message", "Failed to unregister server") if response else "No response from server"
    ...
    def set_sharing(self, ip_address, port, is_shared):
        ...
        if response and response.get("status") == "success":
            ...
            # Refresh server lists to update the UI with current data
            self.refresh_server_lists()
    ...
    def get_server_connection_info(self, ip_address, port):
        """Get connection info for a specific server - read-only operation that doesn't modify recent_connection"""
        if not self.authenticated:
            return False, "Not authenticated", []

        # Send request without username to ensure we're only reading data, not modifying it
        response = self.send_command("get_server_connection_info", {
            "ip_address": ip_address,
            "port": port
            # Explicitly NOT sending username to avoid any update operations
        })
