...
class CentralServerClient:
    def __init__(self, parent_app, host=None, port=5001, use_ssl=False):
        """Initialize the central server client (encryption disabled)"""
    ...
    def connect(self):
        """Connect to the central server without encryption"""
        if not self.host:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                # Get encryption key (for protocol compatibility)
                self.encryption_key = wrapped_socket.recv(44)

                # Set dummy cipher suite (not actual encryption)
    ...
    def encrypt(self, data):
        """Dummy encrypt method for compatibility"""
        if isinstance(data, str):
            return data.encode()
        return data

    def decrypt(self, data):
        """Dummy decrypt method for compatibility"""
    ...
    def send_command(self, command_type, data):
        """Send a command to the central server without encryption"""
